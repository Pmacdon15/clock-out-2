...
export default function ManageHours({ initialEntries = [], isAdmin }: ManageHoursProps) {
  ...
  const clockInMutation = useMutation({
    ...
    onSuccess: (res) => {
      ...
      else if ("error" in res) toast.error(res.error || "Failed to clock in");
    },
  });
      ...
      <div className="mt-12 w-full text-left">
        ...
        <div className="space-y-3">
          {initialEntries.slice(0, 5).map((entry) => (
            <div
              key={entry.id}
              className="flex justify-between items-center p-4 border rounded-xl hover:bg-zinc-50 dark:hover:bg-zinc-900 transition-colors"
            >
